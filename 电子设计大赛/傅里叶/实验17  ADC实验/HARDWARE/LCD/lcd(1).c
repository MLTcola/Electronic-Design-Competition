...
void LCD_WR_BYTE(u16 x,u16 y,u8 dat,u16 direction,u16 HV,u16 Print_Color,u16 AntiWhite_Flag,u16 mode)
{
	u8 t1;
//	u16 y0,x0;
//	y0 = y;x0 = x;
	if(AntiWhite_Flag)dat = ~dat;
	for(t1=0;t1<8;t1++)
	{		
		if(direction==0)
		{
			if(dat&0x80)LCD_Fast_DrawPoint(x,y,Print_Color);
			else if(mode==0)LCD_Fast_DrawPoint(x,y,BACK_COLOR);
			dat<<=1;
		}
		else
		{
			if(dat&0x01)LCD_Fast_DrawPoint(x,y,Print_Color);
			else if(mode==0)LCD_Fast_DrawPoint(x,y,BACK_COLOR);
			dat>>=1;
		}
		if(HV==0)y++;
		else x++;
		if(y>=SSD_HOR_RESOLUTION)return;		//超区域了
		if(x>=SSD_VER_RESOLUTION)return;		//超区域了
	}
}
//显示汉字
void LCD_ShowCHineseString(u16 x0,u16 y0,u8* str,u16 offset,u16 AntiWhite_Flag,u16 mode)
{
	u16 t=0,length;
	u16 x,y;
	u16 index,j;
	length = strlen((char*)str)/2;
	...
	for(j=0; j<length; j++)
	{
		...
		{
			if(Hzk[index].name[0] == str[2*j]&&Hzk[index].name[1] == str[2*j+1])
			{	
				x = x0;y = y0;
				for(t=0; t<16; t++)
				{
					LCD_WR_BYTE(x+t,y,Hzk[index].dat[t],1,0,BLUE,AntiWhite_Flag,mode);
				}
				
				x = x0;y = y0+8;
				for(t=0; t<16; t++)
				{
					LCD_WR_BYTE(x+t,y,Hzk[index].dat[t+16],1,0,BLUE,AntiWhite_Flag,mode);
				}
				x0 = x0+16;
			}
			if(x0>=SSD_HOR_RESOLUTION){x0 = 0;y0 = y0+16;}
		} 
	}
}
//绘制函数
void LCD_DrawFunction(u16 x,u16 y,u16* Dat,u16 width,u16 hight,u16 Print_Color)
{
	u16 i;
	POINT_COLOR = Print_Color;
	for(i=1; i<width; i++)
	{
		//LCD_Fast_DrawPoint(x+i,y+hight-Dat[i]*hight/4096,Print_Color);
		LCD_DrawLine(x+i-1,y+hight-Dat[i-1]*hight/4096,x+i,y+hight-Dat[i]*hight/4096);
	}
//	u16 i;
//	for(i=0; i<width; i++)
//	{
...
void LCD_DrawSpectrum(u16 x,u16 y,u16* Dat,u16 width,u16 hight,u16 Print_Color)
{
	u16 i,j;
	for(i=0; i<width; i++)
	{
		for(j=0; j<Dat[i]*hight/4096; j++)
		LCD_Fast_DrawPoint(x+i,y+hight-j,Print_Color);
	}
}
void LCD_DrawGrid(u16 x,u16 y,u16 width,u16 width_scale_num,u16 hight,u16 hight_scale_num,u16 Print_Color)
{
	u16 i,j,k;
	for(i=0; i+x<width; i++)LCD_Fast_DrawPoint(x+i,y+hight-1,Print_Color);//绘制外框
	for(i=0; i+x<width; i++)LCD_Fast_DrawPoint(x+i,y+hight,Print_Color);
	for(i=0; i+x<width; i++)LCD_Fast_DrawPoint(x+i,y+1,Print_Color);
	for(i=0; i+x<width; i++)LCD_Fast_DrawPoint(x+i,y,Print_Color);
	for(i=0; i<hight; i++)LCD_Fast_DrawPoint(x,y+i,Print_Color);
	for(i=0; i<hight; i++)LCD_Fast_DrawPoint(x+1,y+i,Print_Color);
	for(i=0; i<hight; i++)LCD_Fast_DrawPoint(x+width-1,y+i,Print_Color);
	for(i=0; i<hight; i++)LCD_Fast_DrawPoint(x+width-2,y+i,Print_Color);
	
	for(i=0; i<width_scale_num; i++)
	{
		for(j=0; j<hight; j++)
		{
			if(j%2==0)LCD_Fast_DrawPoint(x+width*i/width_scale_num,y+j,Print_Color);
		}
	}
	for(i=0; i<hight_scale_num; i++)
	{
		for(j=0; j<width; j++)
		{
			if(j%2==0)LCD_Fast_DrawPoint(x+j,y+hight*i/hight_scale_num,Print_Color);
		}
	}
}